...
export default function FloatingChatWrapper() {
  ...
  const [msgs, setMsgs] = useState<{ role: 'user' | 'bot'; text: string }[]>([
    { role: 'bot', text: "I'm KwizBot. Ask me about quizzes, scores, or how to host a family game night!" },
  ])
  ...
  return (
    <>
      <motion.button
        ...
        aria-label="Open Kwizzo AI chat"
        style={{ position: 'fixed', bottom: 24, right: 24, width: 52, height: 52, borderRadius: '50%',
          ...
          boxShadow: '0 4px 20px rgba(124,58,237,0.35)', zIndex: 1000, fontSize: 20 }}
      >
        {open ? '✕' : '🎯'}
      </motion.button>
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            style={{ position: 'fixed', bottom: 88, right: 24, width: 320, height: 420,
              ...
              overflow: 'hidden', boxShadow: '0 8px 40px rgba(124,58,237,0.15)' }}
          >
            ...
            <div style={{ padding: '10px 12px', borderTop: '1px solid #f1f5f9', display: 'flex', gap: 8 }}>
              <input value={input} onChange={e => setInput(e.target.value)}
                ...
                placeholder="Ask about quizzes, topics, game night…"
              ...
              <button onClick={send} disabled={loading}
                style={{ background: '#7c3aed', border: 'none', borderRadius: 8, padding: '7px 13px', fontSize: 13, color: '#fff', cursor: 'pointer', fontWeight: 700 }}>→</button>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
